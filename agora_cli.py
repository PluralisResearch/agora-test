...
DOCKER_IMAGE = "ghcr.io/pluralisresearch/agora-test:latest"
...
# Distribution name -> source directory in this checkout.
SOURCE_PACKAGES = {"agora_server": "agora_server", "agora": "agora", "pluralis-pithos": "pithos"}
...
def parse_announce_ip(value: str) -> str:
    """Argparse type: validate an IPv4 address and return it in canonical form."""
    try:
        return str(ipaddress.IPv4Address(value.strip()))
    except ValueError:
        raise argparse.ArgumentTypeError(f"invalid IPv4 address: '{value}'") from None
...
def detect_missing_packages() -> list[str]:
    """Return the SOURCE_PACKAGES distributions that are not installed."""
    missing = []
...
def install_from_source(packages: list[str]):
    if not has_correct_python():
        error(
            f"Python {PYTHON_VERSION_STR} is required, but you are running "
            f"{sys.version_info[0]}.{sys.version_info[1]}."
        )
        error(f"Please activate a Python {PYTHON_VERSION_STR} environment first.")
        sys.exit(1)
    ensure_pip_version()
    os.chdir(ROOT_DIR)
    ...
    run_cmd(
        ...
        f"Failed to install PyTorch. Re-run with 'pip install torch=={TORCH_VERSION} --index-url https://download.pytorch.org/whl/cu128' to see full output.",
        capture_output=True,
    )
    constraints = str(ROOT_DIR / "constraints.txt")
    for pkg in packages:
        src = f"./{SOURCE_PACKAGES[pkg]}"
        info(f"Installing {pkg}... (this may take a few minutes)")
        run_cmd(
            [sys.executable, "-m", "pip", "install", "--build-constraint", constraints, "-e", src],
            f"pip install {pkg} failed. Re-run with 'pip install --build-constraint constraints.txt -e {src}' to see full output.",
            capture_output=True,
        )
...
def ensure_python_environment(skip_input: bool):
    """Ensure we're in a suitable Python environment for native mode.

    If correct Python is available, do nothing.
    Otherwise, offer to create a conda env with the required Python.
    """
    if has_correct_python():
        return

    warn(f"Python {PYTHON_VERSION_STR} is required, but you are running {sys.version_info[0]}.{sys.version_info[1]}.")

    current_env = os.environ.get("CONDA_DEFAULT_ENV") or os.environ.get("VIRTUAL_ENV")
    if current_env:
        warn(f"Your current environment '{current_env}' does not have Python {PYTHON_VERSION_STR}.")

    if skip_input:
        error(f"Wrong Python version. Create a conda/venv environment with Python {PYTHON_VERSION_STR}.")
        sys.exit(1)
    ...
    if conda_available:
        ...
        info(f"Conda is available. A conda environment with Python {PYTHON_VERSION_STR} can be created.")
        if ask_yn(f"Would you like to create a conda environment 'agora' with Python {PYTHON_VERSION_STR}? [Y/n] "):
            run_cmd(
                ["conda", "create", "-y", "-n", "agora", f"python={PYTHON_VERSION_STR}"],
    ...
    if conda_available:
        activate_cmd = " && ".join(
            [f"conda create -n agora python={PYTHON_VERSION_STR}"] + _conda_activate_instructions("agora")
        )
        print(f"  {activate_cmd}")
        error(f"Cannot continue without Python {PYTHON_VERSION_STR}.")
        sys.exit(1)
    print(f"  python{PYTHON_VERSION_STR} -m venv .venv && source .venv/bin/activate")
    error(f"Cannot continue without Python {PYTHON_VERSION_STR}.")
    sys.exit(1)
...
def ensure_installed(use_docker: bool, skip_input: bool = False):
    ...
    if use_docker:
        ...
        if missing:
            ...
            install_str = " && ".join(
                f"pip install --build-constraint constraints.txt -e ./{SOURCE_PACKAGES[pkg]}" for pkg in missing
            )
            if skip_input:
...
def cmd_start(args):
    ...
    for key in ("host_port", "announce_port", "announce_ip", "log_file", "identity_path", "batch_size_override"):
        value = getattr(args, key, None)
        if value is not None:
            config[key] = value
    ...
    # Auto-generate a per-run log filename so consecutive runs don't overwrite each
    # other; an explicit --log_file is respected verbatim (see _link_latest_log).
    if "log_file" not in config:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        config["log_file"] = f"logs/server_gpu{gpu_id}_{timestamp}.log"
        config["log_file_link"] = f"logs/server_gpu{gpu_id}.log"
    ...
    if announce_ip:
        # Re-validate here: the value may come from a hand-edited config file, not only from argparse.
        try:
            ip = ipaddress.IPv4Address(str(announce_ip).strip())
        except ValueError:
            error(f"Invalid announce IP: '{announce_ip}'")
            sys.exit(1)
        info(f"Using manually specified public IP: {ip}")
        if not ip.is_global:
            warn(f"{ip} is not a globally routable address; other peers may not be able to reach you.")
        public_ip = str(ip)
    else:
    ...
    # Native mode writes the log relative to the CLI's cwd; Docker writes it into
    # ROOT_DIR via the bind mount. Point the stable symlink at the matching location.
    log_base = ROOT_DIR if config["use_docker"] else Path.cwd()
    _link_latest_log(config, log_base)
...
def _link_latest_log(config: dict, base_dir: Path):
    """Point a stable ``server_gpu<ID>.log`` symlink at the newest timestamped log.

    Args:
        config: Run configuration, holding ``log_file`` and, when the log was
            auto-generated, ``log_file_link`` (the stable relative path to create).
        base_dir: Directory the paths resolve against: the CLI's cwd for native mode,
            ``ROOT_DIR`` for Docker (the bind-mount root).
    """
    link_rel = config.get("log_file_link")
    if not link_rel:
        return

    link_path = base_dir / link_rel
    log_name = Path(config["log_file"]).name
    try:
        link_path.parent.mkdir(parents=True, exist_ok=True)
        if link_path.is_symlink():
            link_path.unlink()
        elif link_path.exists():
            # Preserve a real log left by the pre-symlink scheme rather than deleting it.
            mtime = datetime.fromtimestamp(link_path.stat().st_mtime).strftime("%Y%m%d_%H%M%S")
            link_path.rename(link_path.with_name(f"{link_path.stem}_{mtime}{link_path.suffix}"))
        link_path.symlink_to(log_name)
    except OSError as e:
        warn(f"Could not update the latest-log symlink ({e}); the timestamped log is still written.")
...
def _start_docker(config: dict, server_args: list, skip_input: bool = False):
    ...
    inner_cmd = (
        f"CUDA_VISIBLE_DEVICES=0 python{PYTHON_VERSION_STR} {DOCKER_SERVER_PATH} "
        f"{' '.join(server_args)} > agora_output_gpu{gpu_id}.log 2>&1"
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp.add_argument(
        "--announce_ip",
        type=parse_announce_ip,
        metavar="IP",
        help="Advanced: public IPv4 address to announce to other peers (default: auto-detected). "
        "Set only if automatic detection returns the wrong address.",
    )
