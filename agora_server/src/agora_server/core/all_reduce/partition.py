...
from agora_server.hivemind.compression import CompressionBase, CompressionInfo, NoCompression
from agora_server.hivemind.p2p import PeerID
from agora_server.hivemind.proto import runtime_pb2
from agora_server.hivemind.utils import amap_in_executor, as_aiter, get_logger
...
class FailureReason(Enum):
    ...
    SENDER_UNRESPONSIVE = "sender_unresponsive"
...
class TensorPartContainer:
    ...
    def __init__(
        ...
        """Initialize TensorPartContainer.

        Args:
            tensors (Sequence[torch.Tensor]): Local tensors to be split and aggregated.
            peer_fractions (Sequence[float]): For each peer, a target fraction of vector elements that this peer should average.
            peer_ids (Sequence[PeerID]): Identifiers for each peer participating in the aggregation.
            compression (CompressionBase, optional): Optionally compress tensors with this compression algorithm before sending them to peers. Defaults to NoCompression().
            part_size_bytes (int, optional): Greedily split tensors into parts of up to this many bytes (after compression). Defaults to DEFAULT_PART_SIZE_BYTES.
            ...
            return_deltas (bool, optional): If True, output tensors are differences (aggregated tensor - local tensor). Defaults to True.
            prefetch (int, optional): When compressing, pre-compute this many compressed tensors in background. Defaults to 3.
        """
        ...
        self.all_peer_ids = peer_ids
        self.failed_peer_ids = defaultdict(int)
    ...
    def register_failed_reducer(
        ...
        peer_id_str = self.all_peer_ids[peer_index].to_string()
        self.failure_reasons[peer_id_str] = reason
        for part_index in range(self._outputs_registered_by_peer[peer_index], self.num_parts_by_peer[peer_index]):
            ...
            self.failed_peer_ids[peer_id_str] += 1
    ...
    def finalize(self):
        ...
        if not self.finished.is_set():
            ...
            if self.failed_size != 0:
                ...
                for peer_id, failures in self.failed_peer_ids.items():
                    reason = self.failure_reasons.get(peer_id, FailureReason.UNKNOWN)
                    logger.warning(
                        f"Failed to receive tensors from peer {peer_id}: {failures} failures, reason={reason.value}"
                    )
            else:
                logger.info(f"Averaging: received {100:.1f}% results")
            self._outputs_consumed = True
            self.finished.set()
...
class TensorPartReducer:
    ...
    def __init__(self, part_shapes: Sequence[torch.Size], num_senders: int):
        """Initialize TensorPartReducer.

        Args:
            part_shapes (Sequence[torch.Size]): A sequence of shapes of torch tensors that will be averaged by this reducer.
            num_senders (int): Total number of peers in a given all-reduce group that will send gradients.

        Note:
            Even if local peer is not sending data, local parts will be used for shape information.
        """
    ...
    async def accumulate_part(
        ...
        while part_index > self.current_part_index:
            ...
            _, pending = await asyncio.wait(
                {self.current_part_future, finished_task},
        ...
        barrier_wait = time.monotonic() - barrier_start
        if barrier_wait > 0.5:
            logger.warning(
                f"AR barrier wait {barrier_wait:.2f}s on part {part_index} sender {sender_index} "
                f"(current_part_index={self.current_part_index})"
            )
        ...
        if not self.finished.is_set():
            ...
            if self.num_parts != 0 and self.num_senders != 0:
                ...
                if parts_expected != parts_received:
                    ...
                    logger.info(f"Reducer: received {100:.1f}% results")
...
class AllreduceException(Exception):  # TODO: rename to AllReduceError
    """A special exception that is raised when allreduce can't continue normally (e.g. disconnected/protocol error)"""


class BannedException(AllreduceException):  # TODO: rename to BannedError
