...
from agora_server.hivemind.dht.schema import BytesWithPublicKey, RSASignatureValidator, SchemaValidator
...
class TrainingProgressSchema(BaseModel):
    progress: dict[BytesWithPublicKey, LocalTrainingProgress | None]
    ...
    **Example**:
    ...
    def __init__(
        ...
        *,
        ...
        iter_stale_allow: int = 0,
        status_loglevel: int = logging.DEBUG,
        private_key: RSAPrivateKey | None = None,
        daemon: bool = True,
        start: bool,
    ):
        """Initialize the progress tracker for monitoring distributed training collaboration.

        Args:
            dht (DHT): A DHT instance.
            prefix (str): A unique prefix for DHT keys.
            target_batch_size (int): Target global batch size.
            client_mode (bool | None, optional): If True, operate in client mode without accepting incoming connections. Defaults to None.
            min_refresh_period (float, optional): Wait for at least this many seconds before fetching new collaboration state. Defaults to 0.5.
            max_refresh_period (float, optional): Wait for at most this many seconds before fetching new collaboration state. Defaults to 10.
            default_refresh_period (float, optional): If no peers are detected, attempt to fetch state this often (seconds). Defaults to 3.
            expected_drift_peers (float, optional): Assume that this many new peers can join between epochs. Defaults to 3.
            expected_drift_rate (float, optional): Assumes that this fraction of current collaboration can join/leave between epochs. Defaults to 0.2.
            performance_ema_alpha (float, optional): Smoothing value used to estimate this peer's performance (samples per second). Defaults to 0.1.
            metadata_expiration (float, optional): Peer's metadata (e.g. samples processed) is stored onto DHT for this many seconds. Defaults to 60.0.
            status_loglevel (int, optional): Logging level for status messages. Defaults to logging.DEBUG.
            private_key (RSAPrivateKey | None, optional): Private key for signing peer metadata. Defaults to None.
            daemon (bool, optional): If True, run background threads as daemon threads. Defaults to True.
            start (bool): If True, start the tracker immediately upon initialization.

        Note:
            The expected collaboration drift parameters are used to adjust the frequency with which this optimizer will refresh the collaboration-wide statistics (to avoid missing the moment when peers transition to the next epoch).
        """
        ...
        self.iter_stale_allow = iter_stale_allow

        # Owner-tag subkey for this peer's progress records. On a RedisDHT the tag
        # comes from the DHT handle itself (the marker of the key its envelopes are
        # signed with), so it can never diverge from the envelope
        # pubkey the Metadata Store verifies against. On a real DHT (or an unsigned
        # RedisDHT, where the property is None) keep the validator path: RedisDHT's
        # add_validators is a no-op, so registering them there is harmless.
        dht_owner_tag = getattr(dht, "local_public_key", None)
        if dht_owner_tag is not None:
            self._local_public_key = dht_owner_tag
        else:
        ...
        self.allow_progress_report = False
    ...
    def update_epoch(self, new_epoch: int | None = None) -> int:
        ...
        try:
            while not self.shutdown_triggered.is_set():
                ...
                if self.should_report_progress.is_set():
                    ...
                    triggered_by_metadata_expiration = False
                else:
                    logger.debug("Progress update triggered by metadata_expiration")
                    triggered_by_metadata_expiration = True

                # Report progress even if no samples were processed
                if self.allow_progress_report:
                    if not triggered_by_metadata_expiration and self.local_progress.samples_accumulated == 0:
                        logger.debug("Skipping progress report because no samples were processed")
                        continue
                    ...
                    store_task = asyncio.create_task(
                        asyncio.wait_for(
                            self.dht.store(
                                ...
                                value=self.local_progress.dict(),
                    ...
                    await asyncio.sleep(1.0)  # wait a bit to allow the flag to be set to True
    ...
    def _parse_swarm_progress_data(self, metadata: TrainingProgressSchema) -> GlobalTrainingProgress:
        ...
        for peer in valid_peer_entries:
            ...
            if peer.epoch >= (global_epoch - self.iter_stale_allow):
        ...
        if total_samples_accumulated == 0:
            # Report local progress as global if other peers didn't process any batches
            samples_remaining_to_next_epoch = max(0, self.target_batch_size - self.local_progress.samples_accumulated)
            local_eta_next_epoch = samples_remaining_to_next_epoch / self.performance_ema.samples_per_second
