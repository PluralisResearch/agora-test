...
from agora_server.hivemind.dht.crypto import DHTRecord, RecordValidatorBase
from agora_server.hivemind.dht.protocol import DHTProtocol
from agora_server.hivemind.dht.routing import DHTID, BinaryDHTValue, DHTKey, DHTValue, Subkey, get_dht_time
from agora_server.hivemind.dht.storage import DictionaryDHTValue
from agora_server.hivemind.dht.traverse import traverse_dht
from agora_server.hivemind.p2p import P2P, PeerID
from agora_server.hivemind.utils import MSGPackSerializer, SerializerBase, get_logger
from agora_server.hivemind.utils.auth import AuthorizerBase
from agora_server.hivemind.utils.multiaddr import Multiaddr
from agora_server.hivemind.utils.timed_storage import DHTExpiration, TimedStorage, ValueWithExpiration
...
class DHTNode:
    ...
    * find - request one or several keys, get values and expiration (if peer finds it locally) and :bucket_size: of
    ...
    cache_refresh_task: asyncio.Task | None; cache_refresh_evt: asyncio.Event; cache_refresh_queue: CacheRefreshQueue
    blacklist: Blacklist
    # fmt: on
    ...
    async def create(
        ...
        record_validator: RecordValidatorBase | None = None,
        authorizer: AuthorizerBase | None = None,
    ...
    async def find_nearest_nodes(
        ...
        node_to_peer_id: dict[DHTID, PeerID] | None = None,
    ...
    async def store(
        self, key: DHTKey, value: DHTValue, expiration_time: DHTExpiration, subkey: Subkey | None = None, **kwargs
    ) -> bool:
        """
    ...
    async def store_many(
        ...
        expiration_time: DHTExpiration | list[DHTExpiration],
        subkeys: Subkey | list[Subkey | None] | None = None,
        exclude_self: bool = False,
    ...
    async def get(self, key: DHTKey, latest=False, **kwargs) -> ValueWithExpiration[DHTValue] | None:
        """
    ...
    async def get_many(
        self, keys: Collection[DHTKey], sufficient_expiration_time: DHTExpiration | None = None, **kwargs
    ) -> dict[DHTKey, ValueWithExpiration[DHTValue] | None | Awaitable[ValueWithExpiration[DHTValue] | None]]:
        """
    ...
    async def get_many_by_id(
        ...
        sufficient_expiration_time: DHTExpiration | None = None,
        num_workers: int | None = None,
        beam_size: int | None = None,
        return_futures: bool = False,
        _is_refresh=False,
    ) -> dict[DHTID, ValueWithExpiration[DHTValue] | None | Awaitable[ValueWithExpiration[DHTValue] | None]]:
        """
    ...
    async def _refresh_routing_table(self, *, period: float | None) -> None:
...
class _SearchState:
    ...
    binary_value: BinaryDHTValue | DictionaryDHTValue | None = None
    expiration_time: DHTExpiration | None = None  # best expiration time so far
    source_node_id: DHTID | None = None  # node that gave us the value
    future: asyncio.Future[ValueWithExpiration[DHTValue] | None] = field(default_factory=asyncio.Future)
    serializer: type[SerializerBase] = MSGPackSerializer
    record_validator: RecordValidatorBase | None = None

    def add_candidate(
        self,
        candidate: ValueWithExpiration[BinaryDHTValue | DictionaryDHTValue] | None,
        source_node_id: DHTID | None,
    ):
