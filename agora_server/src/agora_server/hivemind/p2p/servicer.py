...
from agora_server.hivemind.p2p.p2p_daemon import P2P
from agora_server.hivemind.p2p.p2p_daemon_bindings.datastructures import PeerID
...
class StubBase:
    ...
    def __init__(self, p2p: P2P, peer: PeerID, namespace: str | None):
        self._p2p = p2p
        self._peer = peer
...
class ServicerBase:
    ...
    _rpc_handlers: list[RPCHandler] | None = None
    _stub_type: type[StubBase] | None = None
    ...
    def _make_rpc_caller(cls, handler: RPCHandler):
        ...
        async def caller(self: StubBase, input: input_type, timeout: float | None = None) -> output_type:
    ...
    async def add_p2p_handlers(
        self, p2p: P2P, wrapper: Any = None, *, namespace: str | None = None, balanced: bool = False
    ) -> None:
    ...
    async def remove_p2p_handlers(self, p2p: P2P, *, namespace: str | None = None) -> None:
        self._collect_rpc_handlers()
    ...
    @classmethod
    def get_stub(cls, p2p: P2P, peer: PeerID, *, namespace: str | None = None) -> StubBase:
        cls._collect_rpc_handlers()
        return cls._stub_type(p2p, peer, namespace)

    @classmethod
    def _get_handle_name(cls, namespace: str | None, method_name: str) -> str:
