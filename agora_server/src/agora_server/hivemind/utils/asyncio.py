...
from typing import AsyncIterable, AsyncIterator, Optional, Tuple, TypeVar, Union
...
async def anext(aiter: AsyncIterator[T]) -> T | StopAsyncIteration:
...
async def aiter_with_timeout(iterable: AsyncIterable[T], timeout: float | None) -> AsyncIterator[T]:
...
def cancel_task_if_running(task: asyncio.Task | None) -> None:
