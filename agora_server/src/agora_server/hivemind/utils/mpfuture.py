...
class MPFuture(base.Future, Generic[ResultType]):
    ...
    _global_sender_pipe: PipeEnd | None = None  # a pipe that is used to send results/exceptions to this process
    _pipe_waiter_thread: threading.Thread | None = None  # process-specific thread that receives results/exceptions
    _active_futures: dict[UID, ref[MPFuture]] | None = None  # non-done futures originated from this process
    _active_pid: PID | None = None  # pid of currently active process; used to handle forks natively
    ...
    def result(self, timeout: float | None = None) -> ResultType:
